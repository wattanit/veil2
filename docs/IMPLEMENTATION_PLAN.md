...
| P7.4 | Not yet built | `--group` on `list` becomes an optional-value flag: omitted stays flat (unchanged), bare stays folder-grouped (unchanged), `--group=extension` groups per FR-29 | FR-29, Spec §5.2 |
...
**Exit:**
- `veil detail` and `veil list --group[=extension]` are covered by CLI tests, table and JSON output both, and a bare `--group` regresses nothing (A-4, backward compatibility per Spec §5.2).
