...
| Item | Work | Cites | Tests |
|---|---|---|---|
| P5.2.a | A pure check — "can this name become a filename without becoming a different name than the vault reports" — added to `veil-core`, taking a name and returning why it fails rather than only that it does | Spec §4.6, FR-31 | T5.5–T5.8 |
...
| P5.2.f | **Revised during implementation** (see the rationale below): rather than dispatching on the host platform, the check enforces the *union* of every supported platform's rules unconditionally — a name Windows reserves is refused on macOS too, and a case collision is refused regardless of whether today's destination happens to be case-sensitive | Spec §8.1, HC-8 | T5.5–T5.8 |
...
**Why P5.2.f is a union rather than a dispatch on the host platform, contrary to how this task was first sketched.** The original plan was `cfg(target_os)`-gated: Windows' rules would apply only when compiled for Windows, and the case-insensitivity check would depend on probing the destination volume. Implementation found two problems with that. First, Spec §4.6's own wording — "not on the extracting platform" — is about the platform that is *actually extracting*, and on macOS today that would mean `CON.txt` extracts just fine, which contradicts the reading this document's test cases were already written against (T5.5, T5.6 refuse it on this development machine). Second, and more fundamentally: making the check's *answer* depend on the host platform would make the check itself a host fact, which is the exact thing HC-8 exists to keep out of Veil2 — applied here to a check rather than to the stored format, but the same defect in miniature. Enforcing the union sidesteps both problems, needs no destination-volume probe (which would have needed the same `mount`-parsing machinery P5.4 builds, creating an ordering dependency between two tasks that do not otherwise need one), and is stricter than strictly necessary only for names nobody plausibly uses (`CON.txt`, a trailing space) — a cost this document judges worth paying for a check whose own behaviour is now identical everywhere. The distinction P5.3.d originally drew between what refuses "here, on macOS" versus "only on a case-insensitive or Windows destination" no longer applies for that reason: everything in the fixture's reserved set now refuses everywhere.
...
| P5.3.d | The Windows-reserved names are stored in the vault (Spec §4.6 places no restriction on what a vault may *hold*; FR-31 restricts what may be *extracted*) and the fixture's manifest records the reason each is expected to refuse with — P5.2.f's union means every one of them refuses here, on macOS, rather than only a platform-specific subset | Spec §4.6, FR-31 | T5.11, T5.13 |
...
1. **FR-31's representability rule names the failure mode — a reserved name, a reserved character, a case collision — but not the exact set, and Spec §4.6's own wording ("not on the extracting platform") reads as a per-platform check, which is not what got built.** *Proposed:* the enumeration in P5.2.b (Windows' `CON`/`PRN`/`AUX`/`NUL`/`COM1`–`9`/`LPT1`–`9`, its reserved characters and control characters, trailing dot or space, and a case collision against another entry in the same folder), enforced as the **union of every supported platform's rules, unconditionally** — refused the same way regardless of which platform is running, per P5.2.f's rationale. This is a departure from §4.6's current wording, not only an enumeration of it, and the departure is the part that most needs a ruling: whether the check should instead read the host platform (or, later, probe the destination volume) and permit a name elsewhere that it refuses here. Resolver: owner, to land in Spec §4.6 — either by adopting the union or by directing P5.2 to dispatch on platform instead, in which case T5.5 and T5.6 change to assert a refusal that depends on `cfg(target_os)`.
...
- **~~Whether P5.2's check belongs in `veil-core` as a public API or stays private to `save_copy`'s call site.~~** Resolved during implementation: exported now, as `Vault::check_representable`, on the reasoning this question itself gave — exporting early costs nothing and a future GUI caller (Phase 6 or 7) is already anticipated by P5.2.a's wording.
- **Whether the fixture's non-ASCII scripts need a *macOS* case-insensitive-volume run in addition to what T5.7 and T5.8 already assert against the vault's own entries.** Now more clearly a coverage question than it was before P5.2.f's revision: the collision check no longer depends on the destination's actual case sensitivity at all, so there is nothing platform-specific left for an HFS+-formatted image to exercise that the unit tests do not already cover. Recorded in case a future revision reintroduces a destination-dependent check. Resolver: owner, when P5.3 is implemented.
