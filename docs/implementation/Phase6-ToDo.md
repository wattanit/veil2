...
**`choose_vault_path`'s `"open"` mode picked a folder, until P6.14's packaging work made that stop working.** Registering `.veil` as a package-conforming UTI (so Finder shows a vault as one document rather than a browsable folder) is a system-wide Launch Services change, not something scoped to just Finder's display — it also changes what macOS's own file-picker APIs will let a folder-only panel select. A `.veil` directory stopped being choosable as a "folder" the moment that registration landed, confirmed live against a real signed build, after the fact: the picker simply couldn't select a vault at all. Fixed by switching `"open"` to a file picker filtered to the `.veil` extension instead — the same way any package-type document (a `.app`, an `.rtfd`) is opened through an Open File panel, never an Open Folder one. `"create"` was already a save-file panel and was never affected.
...
| P6.0.c | Done | `choose_vault_path(mode: "open" \| "create") -> Option<String>` — native dialogs (`tauri_plugin_dialog`, called from Rust, as `choose_save_path` already is), so a path reaches `open_vault`/`create_vault` without the fixture bypass. `"open"` picks a *file* filtered to `.veil`, not a folder — once P6.14's package registration landed, a folder picker could no longer select a vault at all, since macOS stopped presenting `.veil` directories as folders; confirmed live and fixed | Design §5, §8.1, §8.2 | T6.2, T6.3 |
