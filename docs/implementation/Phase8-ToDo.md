...
**Every automatable gate is met.** P8.1 through P8.10 are done; `cargo fmt --check`, `cargo clippy --workspace --all-targets -D warnings`, `cargo test --workspace` (every crate, including this phase's `sort.rs`, `selection.rs`, `damage.rs`, and `structure.rs`'s new T8.21), `cargo deny check`, and `cargo audit` all pass at `v2.1.0`, and both `cargo build --workspace` and the frontend's own `npm run build` succeed. No new dependency was added on either side.

**What this does not yet cover** is the live half of this document's own Conventions §: every rendering-and-interaction case (T8.1–T8.3, T8.5–T8.6, T8.8–T8.22 to the extent they involve looking at the screen) is, by this phase's own stated testing strategy, checked in `cargo tauri dev` by a person looking at it — deliberately deferred to a single pass at the end rather than checked piecemeal after each task, per this phase's own agreement. Per the Plan's Definition of Done (§ Conventions, item 1: "the behaviour... is observable"), Phase 8's exit is not fully met until that pass happens. Three corrections surfaced along the way rather than being caught in review afterward: a Design Guideline overclaim about a toolbar control that never existed (P8.4), a §6 message neither GUI surface had ever actually implemented (P8.8), and an incorrect assumption about which manifest files carry a version field (P8.10).
