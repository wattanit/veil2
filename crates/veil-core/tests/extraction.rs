...
//! (FR-17, FR-18, HC-2, HC-3, S-3, Spec §4.7).
...
/// T2.16 — content survives a round trip through the public API (FR-17).
///
/// Phase 1's T1.10 proved this for the stream. This proves it for the API a
/// frontend actually calls, across the entry-file storage between them —
...
/// T2.17 — a damaged entry produces no output file (FR-18, HC-3, S-3).
///
...
fn t2_17_a_damaged_entry_produces_no_output_file() {
    ...
    harness::flip_byte_in_entry_file(&dir, target, 8);
    ...
    match outcome {
        Err(Error::Corrupt { what, affected }) => {
            ...
            // Named, because S-3 requires a partial failure to be presented as
...
/// T2.19 — extraction writes only where the caller said (HC-2, FR-17).
///
