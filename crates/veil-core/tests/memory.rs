//! Phase 1 test case T1.11 — peak memory does not follow file size
//! (S-1, A-2, Spec §3.3).
//!
...
use veil_core::crypto::{CHUNK_LEN, decrypt, encrypt, generate_dek, generate_nonce_prefix};
...
///
/// A `Vec` of the plaintext would dominate the measurement and turn this into
/// a test of the fixture. The point is what the *cryptographic layer* holds.
struct Generated {
    remaining: usize,
}
...
/// A sink that counts bytes and keeps none.
struct Counted(u64);
...
fn peak_for(len: usize) -> usize {
    ...
    // Encrypt into a temporary file rather than memory, so the measurement is
    // of the streaming layer and not of the buffer holding its output.
    let dir = std::env::temp_dir().join(format!("veil2-t1-11-{len}"));
    ...
    let mut sealed = std::fs::File::open(&dir).unwrap();
    decrypt(&dek, &nonce, 1, None, &mut sealed, &mut Counted(0)).unwrap();
...
/// T1.11 — peak allocation differs by a bounded constant, not by the ratio of
/// the input sizes (S-1, A-2).
#[test]
fn t1_11_peak_memory_does_not_scale_with_content_size() {
    let small = CHUNK_LEN * 2;
    let large = CHUNK_LEN * 32;
    assert_eq!(large / small, 16, "the fixture sizes must differ sharply");

    // Warm up: first-call allocations that belong to neither measurement.
    let _ = peak_for(CHUNK_LEN);

    let peak_small = peak_for(small);
    let peak_large = peak_for(large);

    // Sixteen times the content. If peak memory followed content size, the
    // second figure would be roughly sixteen times the first.
    assert!(
        peak_large < peak_small * 2,
        "peak memory followed content size: {peak_small} bytes for {small}, \
         {peak_large} bytes for {large} — a {}x input gave a {:.1}x peak",
        large / small,
        peak_large as f64 / peak_small as f64
    );

    // And the absolute figure stays within a small multiple of one chunk,
    // which is what makes a 64 GiB entry retrievable on ordinary hardware.
    assert!(
